<template>
  <div id="recommend" v-cloak>
    <div class="header_top">
      <div  v-if="$route.query.theme_id" class="header_title" v-html="card_data.title"></div>
      <div  v-if="$route.query.theme_id" class="time_author">
         
        <span class="daynow">今日热门</span>
        <span class="author">作者：{{card_data.author}}</span>
    ...
    <div class="tip" v-if="$route.query.theme_id">
      <div>
        <span style="color:#c13f46;font-weight: 500;font-size: 4.2vw;">热情提示:</span>
       <span v-html="card_data.content"></span>
      </div>
    </div>
...
export default {
  ...
  created() {
    ...
    if(this.$route.query.defined){//商城轮播跳转到主题
      this.$axios.get("/pdd/pdd/selection?defined="+1,{ //defind=1为主题列表，defind =2为单品
          headers: {
          },
          params: data
        }).then(res=>{
    ...
    window.onscroll = () => {
      ...
      if (this.$route.path == "/recommend") {
        ...
        if (scrollTop > windowHeight) {
          ...
          this.$refs.scroll_top.style.right = "4%";
